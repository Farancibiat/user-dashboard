  ...
  return (
    ...
    >
      ...
      <Descriptions.Item label="Nombre">{store.activeContact[1].name}</Descriptions.Item>
        ...
        <Descriptions.Item label="Dirección" span={2}>{<a target="_blank" href={`https://www.google.com/maps/search/?api=1&query=${store.activeContact[1].address.geo.lat},${store.activeContact[1].address.geo.lng}`}>{`${store.activeContact[1].address.street}, ${store.activeContact[1].address.suite}`}</a>}</Descriptions.Item>
      </Descriptions>
